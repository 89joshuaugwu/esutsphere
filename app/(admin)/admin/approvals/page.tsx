...
import { getAllPendingUsers, approveUser, rejectUser } from "@/lib/admin";
import type { User } from "@/types";
import {
  Check, X, Eye, FileText, Clock, Loader2, Search,
...
export default function ApprovalsPage() {
  ...
  async function loadPending() {
    setLoading(true);
    const res = await getAllPendingUsers();
  ...
  const handleApprove = async (u: User) => {
    ...
    setProcessing(u.uid);
    try {
      await approveUser(u.uid, admin.uid, admin.displayName, admin.role as 'super_admin');
      toast.success(`${u.displayName} approved successfully!`);
  ...
  const handleReject = async () => {
    ...
    try {
      await rejectUser(rejectModal.uid, reason, admin.uid, admin.displayName, admin.role as 'super_admin');
  ...
  const handleBulkApprove = async () => {
    ...
    const toApprove = pending.filter(u => selected.has(u.uid));
    for (const u of toApprove) {
      try {
        await approveUser(u.uid, admin.uid, admin.displayName, admin.role as 'super_admin');
      } catch { /* continue */ }
    }
    toast.success(`${toApprove.length} users approved`);
    setPending(prev => prev.filter(p => !selected.has(p.uid)));
    setSelected(new Set());
  };
    ...
    <div style={{ animation: "card-enter 0.4s both" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-7 pb-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        <div>
          <p className="text-[13px] text-text-disabled flex items-center gap-2 mb-2">
            Admin <span className="text-[12px]">›</span> <span className="text-text-primary">Approvals</span>
          </p>
          <h1 className="font-display text-[clamp(24px,2.5vw,32px)] text-text-primary leading-tight mb-1.5">
            Pending Approvals
          </h1>
          <p className="text-[14px] text-text-muted">{pending.length} student{pending.length !== 1 ? "s" : ""} awaiting verification</p>
        </div>
      </div>
        ...
        {selected.size > 0 && (
          <div className="flex gap-2 ml-auto">
            <button
              ...
              className="h-[38px] px-4 rounded-[9px] text-[13px] font-semibold flex items-center gap-2 transition-all bg-success/15 border border-success/30 text-success hover:bg-success/[0.22]"
            >
              <Check className="w-4 h-4" /> Approve {selected.size}
            </button>
            <button
              onClick={() => setSelected(new Set())}
              className="h-[38px] px-3 rounded-[9px] text-[13px] text-text-disabled hover:text-text-primary transition-colors"
            >
              Clear
            </button>
          </div>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[14px] text-text-disabled max-w-[300px]">No pending approvals right now. New registrations will appear here.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((u, i) => (
            <div
              key={u.uid}
              className={`rounded-2xl p-5 transition-all ${selected.has(u.uid) ? "border-brand/[0.35] bg-brand/[0.04]" : "hover:border-white/[0.12]"}`}
              style={{
                background: "rgba(15,15,26,0.8)",
                border: `1px solid ${selected.has(u.uid) ? "rgba(124,58,237,0.35)" : "rgba(255,255,255,0.08)"}`,
            ...
            >
              ...
              <div className="flex items-start gap-3.5 mb-4">
                <input
                  ...
                  className="w-4 h-4 rounded accent-brand mt-1 cursor-pointer shrink-0"
                />
                <img
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    <span className="text-[11px] font-semibold px-2 py-[3px] rounded-full bg-cyan/10 text-cyan border border-cyan/20">
                      {u.department}
                    </span>
                    <span className="text-[11px] font-semibold px-2 py-[3px] rounded-full bg-brand/10 text-brand-light border border-brand/20">
                      {u.currentLevel}
                    </span>
                  </div>
                </div>
              ...
              {/* Info Grid */}
              <div
                className="grid grid-cols-3 gap-3 p-3.5 rounded-xl mb-4"
                style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}
              >
                <div>
                  <p className="text-[10px] font-bold text-text-disabled uppercase tracking-[0.5px] mb-1">Matric No.</p>
                ...
                <div>
                  <p className="text-[10px] font-bold text-text-disabled uppercase tracking-[0.5px] mb-1">Year of Entry</p>
              ...
              <div className="flex items-center gap-2.5 pt-3.5" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                {u.admissionLetterUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="h-[34px] px-3.5 rounded-lg text-[13px] font-medium flex items-center gap-2 transition-all bg-white/[0.05] border border-white/10 text-text-muted hover:bg-white/[0.09] hover:text-text-primary"
                  >
                ...
                <button
                  ...
                  disabled={processing === u.uid}
                  className="flex-1 max-w-[180px] h-10 rounded-[10px] text-[14px] font-bold flex items-center justify-center gap-2 transition-all bg-success/[0.12] border border-success/30 text-success hover:bg-success/[0.22] hover:-translate-y-[1px] hover:shadow-[0_6px_18px_rgba(16,185,129,0.2)] disabled:opacity-50"
                >
      ...
      {rejectModal && (
        ...
        >
          <div
            className="w-full max-w-[440px] p-8 rounded-[20px]"
            style={{
              background: "#1E1E35",
          ...
          >
            ...
            >
              <img
                src={rejectModal.profilePicture || `https://api.dicebear.com/7.x/avataaars/svg?seed=${rejectModal.username}`}
                alt={rejectModal.displayName}
                className="w-10 h-10 rounded-full object-cover"
              />
              <div>
                <p className="text-[14px] font-semibold text-text-primary">{rejectModal.displayName}</p>
                <p className="text-[12px] text-text-muted">{rejectModal.matricNumber} · {rejectModal.department}</p>
              </div>
            </div>

            {/* Reason Select */}
            <label className="block text-[13px] font-medium text-text-secondary mb-1.5">Rejection Reason</label>
            <select
              value={rejectReason}
              onChange={(e) => setRejectReason(e.target.value)}
              className="w-full h-11 px-3.5 mb-3 rounded-[10px] bg-white/[0.04] border border-white/10 text-[14px] text-text-primary focus:border-error/50 outline-hidden transition-all"
            >
            ...
            <div className="flex gap-3 mt-4">
              ...
              <button
                ...
                disabled={processing === rejectModal.uid || (!rejectReason || (rejectReason === "custom" && !customReason))}
                className="flex-1 h-11 rounded-[10px] text-[14px] font-bold text-error transition-all disabled:opacity-40 disabled:cursor-not-allowed bg-error/15 border border-error/35 hover:bg-error/25"
              >
